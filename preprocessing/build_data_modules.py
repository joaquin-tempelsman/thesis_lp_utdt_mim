...
def get_precios_scrapped(fecha_inicio, input):
    ...
    df_scrapping["periodo"] = round(
        (pd.to_datetime(df_scrapping["periodo_inicio"], format='%d/%m/%Y') - pd.to_datetime(fecha_inicio, format='%d/%m/%Y'))
        / np.timedelta64(1, "M"),
        0,
    ).astype(int)
    ...
    df_precios["PERIODO_INICIO"] = pd.to_datetime(df_precios["PERIODO_INICIO"], format='%d/%m/%Y')
    ...
    for periodo, edad_animal, clase in itertools.product(
        ...
        try:
            ...
            if edad_animal in [6, 7, 8]:
                if clase == 1:
                    precio = float(row["VAQUILLONAS270"]) * peso_prom_destete * PARAMS["multiplicador_destete"]
                if clase == 2:
                    precio = float(row["NOVILLITOS300"]) * peso_prom_destete * PARAMS["multiplicador_destete"]
            ...
            # momento 2
                ...
                precio = int(row["VAQUILLONAS270"]) * peso_prom_vaquillonas * PARAMS['multiplicador_c3']
...
def write_params_file(PATH_DAT_FILES, PARAMS, SALES_PERIODS):
    ...
    with open(PATH_DAT_FILES["agosto_si"], "w") as f_yes, open(PATH_DAT_FILES["agosto_no"], "w") as f_no:
        num = 8
...
def append_daily_income_and_cost(df_ventas, df_precios, PARAMS, PESOS_PROMEDIO):
    df_ventas = df_ventas.loc[(df_ventas.FECHA >= PARAMS['fecha_inicio']) & (df_ventas.FECHA <= PARAMS['fecha_fin_ejercicio'])]
    for index, row in df_ventas.iterrows():
        precios = get_precios_del_periodo(row.FECHA, df_precios)
    
        # SALES_EARNINGS
        earned_VAQUILLONAS270 = row["VAQUILLONAS270"] * precios["VAQUILLONAS270"] * PESOS_PROMEDIO['peso_prom_vaquillonas']
        earned_NOVILLITOS391 = row["NOVILLITOS391"] * precios["NOVILLITOS391"] * PESOS_PROMEDIO['peso_prom_novillos_pesados']
        earned_NOVILLITOS300 = row["NOVILLITOS300"] * precios["NOVILLITOS300"] * PESOS_PROMEDIO['peso_prom_novillitos']
        ...
        # SALES COSTS
        cost_VAQUILLONAS270 = row["VAQUILLONAS270"] * PARAMS['costos_meses_usd_c1_c2'][17]
        ...
        cost_TERNEROS_DESTETE = (
            row["TERNEROS_DESTETE"]
            * PARAMS['costos_meses_usd_c1_c2'][6]
        )
...
def quote_stock(prices, stock_row, PESOS_PROMEDIO, costs_interpolator):
    ...
    VACAS_sell =stock_row['VACAS'] = prices['VAQUILLONAS270'] * 0.5 * PESOS_PROMEDIO['peso_prom_vaquillonas']
    VAQ12_sell =stock_row['VAQ 1-2'] = prices['VAQUILLONAS270'] * PESOS_PROMEDIO['peso_prom_vaquillonas']
    VAQ12s_sell =stock_row['VAQ. 1-2 Servicio'] = prices['VAQUILLONAS270'] * 0.5 * PESOS_PROMEDIO['peso_prom_vaquillonas']
    VAQ23_sell =stock_row['VAQ. 2-3'] = prices['VAQUILLONAS270'] * 0.5 * PESOS_PROMEDIO['peso_prom_vaquillonas']
    NOVILLOS_sell =stock_row['NOVILLOS'] = prices['NOVILLITOS391'] * PESOS_PROMEDIO['peso_prom_novillos_pesados']
    NOVILLITOS_sell =stock_row['NOVILLITOS'] = prices['NOVILLITOS300'] * PESOS_PROMEDIO['peso_prom_novillitos']
    TERNEROS_sell =stock_row['TERNEROS'] = prices['NOVILLITOS300'] * 1.2 * PESOS_PROMEDIO['peso_prom_destete']
    TERNERAS_sell =stock_row['TERNERAS'] = prices['NOVILLITOS300'] * 1.2 * PESOS_PROMEDIO['peso_prom_destete']
    OREJANO_MACHOS_sell =stock_row['OREJANO_MACHOS'] = 0
    OREJANO_HEMBRAS_sell =stock_row['OREJANO_HEMBRAS'] = 0

    # COST
    VACAS_cost = get_month_cost([x for x in range (13)] + [12] * (66-12), costs_interpolator)
    VAQ12_cost = get_month_cost([x for x in range (13)] + [12] * (14-12), costs_interpolator)
    VAQ12s_cost = get_month_cost([x for x in range (13)] + [12] * (14-12), costs_interpolator)
    VAQ23_cost = get_month_cost([x for x in range (13)] + [12] * (12-30), costs_interpolator)
    ...
    categories = [('VACAS', VACAS_sell, VACAS_cost),
    ...
    df_final_stock_value = pd.DataFrame(categories, columns=['cat','sell_value', 'cost'])
    df_final_stock_value['revenue'] = df_final_stock_value['sell_value'] - df_final_stock_value['cost']

    return df_final_stock_value

def business_exercise_value(df_ventas, df_precios, PARAMS, path_parte_diario, PESOS_PROMEDIO, costs_interpolator):
    fecha_fin_ejercicio = PARAMS['fecha_fin_ejercicio']

    ## -- sales to value -- ##
    df_ventas_cut = df_ventas[(df_ventas['FECHA'] >= PARAMS['fecha_inicio']) & (df_ventas['FECHA'] <= fecha_fin_ejercicio)]
    ...
    log.info(f"moved {(fecha_fin_ejercicio - init_date).days} from {init_date} to find data in stock diario file")
    
    prices_final_period = get_precios_del_periodo(fecha_fin_ejercicio, df_precios).to_dict()   

    df_final_stock_value  = quote_stock(prices_final_period, end_stock_row, PESOS_PROMEDIO, costs_interpolator)
